...
def get_list_data(db: Session, list_id: int) -> NewListGet:
    ...
    return NewListGet(title=db_list.list_title , priority=db_list.list_priority , tasks=task_services.get_tasks_by_list_id(db , list_id))

def create_list(db: Session , list: NewListCreate) -> NewListCreate:
    db_list = Todo_List(**list.model_dump())
    db.add(db_list)
    db.commit()
    return list

def edit_list(db: Session, list_id: int, list: NewListEdit) -> NewListCreate:
    ...
    db.commit()
    db.refresh(db_list)
    return NewListCreate(title=db_list.title , id_priority=db_list.id_priority)
